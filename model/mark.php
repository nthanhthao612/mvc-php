...
class mark{
    ...
    public function authentication($magv,$malop,$mamh){
        $sql1 = "SELECT gv.magv,gv.tengv,gv.hotendemgv,l.malop,gv.mamh,mh.tenmh,bd.mahk FROM giaovien AS gv,monhoc AS mh, bangdiem AS bd, lop AS l,hocsinh AS hs WHERE mh.mamh = gv.mamh AND hs.malop = '$malop' AND gv.magv = bd.magv AND hs.malop = l.malop AND bd.mahs = hs.mahs GROUP BY gv.magv";
        $list = $this->db->selectALot($sql1);
        foreach($list as $value){
            if($value['magv'] == $magv AND $value['malop'] == $malop AND $value['mamh'] == $mamh)
                return TRUE;
        }
        return FALSE;
    }
